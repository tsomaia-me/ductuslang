...
- TRAITS BATCH (D-12 + D-13) APPROVED for execution (owner, 2026-07-15). D-13's two residual defaults CONFIRMED not vetoed: (1) effect methods stay in the effective method set for collision purposes, excluded only at the dispatch-candidate step; (2) the three-call-forms rule carves out effect methods (value methods only; effect methods reached via effects:/|>).

- SATISFIES-RETIREMENT: RULED (owner, 2026-07-16) — option (d), FULL RETIREMENT of the `satisfies` clause. Conformance is declared by the `fulfill` block alone, local or foreign alike (orphan rule + sealing gate who may write one). Rationale chain on record: the fulfill-without-satisfies waiver was pre-audit law, backwards from owner intent; fixing it asymmetrically (module-boundary rule) would bind only the owner module while exempting foreign ones; retirement dissolves the asymmetry, the waiver, 005-65's exception list, 005-66, the D-12 waiver half, and the associated-type flag. Visibility concern absorbed by @derive (visible at the type, directive on its own line ABOVE the declaration) + documentation tooling (Rust/Haskell precedent). REFINEMENTS RULED (owner, 2026-07-16): (1) OPEN-BY-DEFAULT — types are open to foreign conformance; the closing marker reuses `sealed` (`sealed type Song:` = only Song's module may conform Song to traits; mirror of `sealed trait`). (2) BODILESS FULFILL — `fulfill Trait for Type` with no colon/body is legal (where-clause permitted); it is the marker-trait claim form, the no-override form for defaulted traits, and an optional explicit pin for auto-satisfied traits. (3) KEYWORD: `fulfill` KEPT — its anchor is the trait-as-contract metaphor, which survives. CRITICAL EXECUTION DISTINCTION: only the satisfies CLAUSE/keyword retires; 'satisfies'/'satisfy' as ordinary English verbs in rules (bounds satisfaction, requirement satisfaction, auto-satisfaction as a concept) SURVIVE — editors must classify clause-uses vs verb prose per site. Execution: own amendment plan document after refinements settle; blast radius includes §005 wholesale, node-body satisfies clauses, sealed both-doors→one-door, collision rule re-anchor to the fulfilled-set, a per-(trait,type) single-fulfill coherence rule, and the just-landed D-12 waiver text.

- SATISFIES-RETIREMENT FLAG RULINGS (owner, 2026-07-16): (1) R5 site-list HARMONIZED BY COVERAGE (a) — the legal fulfill sites are exactly the modules the orphan rule admits: declaring the trait, declaring the type's head constructor, or covering the pair through a local type argument (010-20) or tuple element (012-64); R5's load-bearing content is unchanged — at most one fulfill per (trait, type) pair, error naming every offending site. (2) GROUNDED AUTO-SATISFACTION (a) — auto-satisfaction applies only to a trait with at least one satisfied requirement or defaulted member; a completely empty trait (no methods, no associated types, no requires) never auto-satisfies and takes an explicit bodiless fulfill claim; pure-requirement umbrellas keep auto-satisfying. (3) SEALED IS UNIFORM over nominal conformable declaration kinds — sealed type / sealed enum / sealed node / sealed connection (newtypes via the type head); the structural carve-outs (tuples, closures) are conformable through element coverage but NOT sealable: no declaration to modify, no declaring module whose exclusive right sealing would protect.

- STRUCTURE BATCH (D-09 + D-14 + D-16, D-24 as gate) APPROVED for execution (owner: "proceed with this batch", 2026-07-16). D-09 defaults confirmed (P020 mislabel reading; P021 third form relabeled WeakHandle); D-14 defaults confirmed (widen the bound; StringifiableKey and the Map key bound defined as ONE canonical set); D-16 default confirmed (bundle.count = row count, intentional element-tally exception). Handle stays a bracket TYPE (owner Q&A: storable value, razor's positive case; handle/handle! keywords are the acquisition operations). The two riders (AssocTypeBinding '='->'is'; SPEC §13.3.7 at-most-once list) were NOT nodded — excluded, still queued.

- OBSERVE-GRANULARITY: RULED (owner, 2026-07-16) — option (A): an observe arm with a stream trigger fires ONCE PER PENDING EVENT, in commit order. Stream context: one output event per input event (consistent with the existing per-event stream-expression law). Value context: the firings CHAIN within the commit — the second and later firings see the running result — and the cell commits the final value; the chaining rule must pin that a self-reference on 2nd+ firings within one commit reads the RUNNING value while `.previous` keeps meaning previous COMMITTED value across commits. Grounds the expressibility of accumulate/event_count/any/all (the stdlib classification); no event is silently unobserved (cursor law + loss-law spirit). ALSO: D-18's old F128 residual (read-site rejection mechanism, (a) vs (b)) is DISSOLVED — superseded by the stream eviction: a stream never binds to `cell T`, so no read site exists; record at streams-batch execution. D-19 executes as ruled (DRAIN; completion = downstream emission only; merge until all arms complete; derived consumers unaffected; observation cells never surface completion). STREAMS BATCH now fully specified; awaits structure-batch landing + owner commit.

- D-05 CONSENT-LIST DISPOSED (owner, 2026-07-16): items 1 (ownership keywords class: own/move), 3 (kind keywords class: cell first member; reactive declaration keywords double as kind heads without re-listing), 4 (declaration-modifier class: sealed), 5 (ruled Model-A assignments execute: enum->declaration; requires/wraps/effects->clause; given/observe->control-flow/selector; collect/fold/yield restored to SPEC paragraph), 6 (FULL sweep; LOG classes reconcile with GRAMMAR §2.4 as ONE taxonomy), 7 (satisfies + main both freed silently; ZERO reservation/migration mentions of them as keywords remain — for satisfies the clause is already extinct and the English VERB survives per the standing razor; main's total de-mention executes with the parked main-removal amendment) — ALL APPROVED. Item 2 REJECTED with owner counter-proposal: BARREL-VISIBILITY redesign — retire in-source `public`; a library's public API = a single barrel file (public.duc) at the package root listing exported names with a keyword; `shared` questioned as a keyword (it is the default). Pending design discussion; D-05's visibility-class item PARKED behind it; the policies batch proceeds with the six approved items.

- BARREL-VISIBILITY RULINGS (owner, 2026-07-16): (1) LEAK RULE = compile error — the exported surface may reference only exported (or built-in) types. (2) ENTRY FORMS: `export audio::Mixer`; `export audio::Mixer as Desk`; grouped `export audio::(Mixer as Desk, SomeOtherThing, SomeOtherOtherThing as Tldr)` — all legal. Glob `export audio::*` PENDING (orchestrator recommends rejection: the barrel is a contract; globs silently grow the API). (3) NO BARREL = NOTHING PUBLIC (apps need no file; library = exports, no roots — mirrors @root). (4) smart-constructor reformulates: barrel exports the type; constructor visibility in source; parenthesized `public(private)` form retires. (5) FILE = public.duc, KEYWORD = `export`, reserved word everywhere (no contextual keywords). (6) blast radius acknowledged — own amendment plan doc after remaining points settle. RAZOR CONFIRMED for both freed words: zero keyword/reservation mentions; English verb 'satisfies' survives; 'main' de-mention total and rides main-removal. ALL PENDING POINTS RULED (owner, 2026-07-16): GLOB REJECTED (`export audio::*` illegal — the barrel is a curated contract); MEMBER MODEL (i) — members keep `public`/`private`: private = module-only, unnamed default = package-visible, member-`public` = part of the exported surface when the enclosing type is barrel-exported (inert otherwise; leak rule applies member-wise; the constructor is an ordinary member — private constructor + exported type = smart-constructor; parenthesized constructor-visibility form retires); BARREL COUNT = ONE PER PACKAGE at the root (`public.duc`; deep entries by path; non-root barrels would be semantically vacuous). `shared` keyword RETIRES everywhere (the unnamed package-visible default). Redesign FULLY SPECIFIED; plan doc = barrel-visibility-plan.md. CONSTRUCTOR-VISIBILITY HOST RULED (owner, 2026-07-16): option (a) — the inner spec relocates onto the type keyword: `type(private) Email:`; unmarked constructors follow the type's full reach (exported type => callable cross-package); package-only-constructor-on-exported-type deliberately unsupported (private constructor + package-visible factory). KEYWORD CONFIRMED: `export` (NOT `expose` — already the node structural-output clause keyword; one word must not carry two unrelated senses). Sequencing: policies batch first, then this amendment. Current law for reference: 003-1..7 (public=cross-package, shared=package DEFAULT unwritten, private=module); members carry the same scheme via FieldVisibility, default shared.

- OBSERVE-GRANULARITY SUPERSEDED -> WORLD B (owner, 2026-07-16, full ruling chain on record): observe observes SIGNALS ONLY (signal-triggered arms fire at most once per commit; chains, .running/.sofar, and the co-firing (Some,None) machinery all dissolve; FLAGS 1/2/3 of the streams batch dissolve structurally). The two stream<->cell crossings become dual LANGUAGE forms spelled with the wiring-arrow as the minting-access operator: `signal->changes` (signal to stream; sequence of commits) and `stream->latest(fallback)` (stream to derived; latest event, fallback before first). Razor: dot PROJECTS existing state (observation cells, .previous/.past stay members); arrow MINTS wiring. `->` gains a third positionally-disjoint role (return types / pairs entries / expression-postfix crossing) — zero parse ambiguity. to_signal RETIRES (one crossing, one spelling; loss-law text and the cannot-read-stream diagnostic repoint to ->latest). Member-form .changes RETIRES (~20 sites). scan-family stays plain stdlib code; accumulate = (src |> scan(init,f))->latest(init) — capability fully preserved, zero privileged operators. Bang-access proposal WITHDRAWN (owner refuted: ! is handle!'s non-Option marker AND the attribute-clause boolean-false form 021-101 — no minting family exists). Names confirmed semantic: changes/latest; DEFAULT pending veto: both reserved words per Model A. D-19's half of the uncommitted streams batch SURVIVES; the granularity half reworks. Full protocol: survey -> plan doc (streams-rework) -> owner approval -> orchestrated execution -> review -> verify chain -> commit.

- STREAMS-REWORK APPROVED for execution (owner, 2026-07-16). Plan doc = streams-rework-plan.md. Defaults confirmed not vetoed: CrossingName closed list (changes/latest NOT reserved — DirectiveName precedent); ->latest name kept; ordinary operator spacing. Jumps the queue ahead of policies (corrects committed semantics). Note on record: the streams batch (D-19 + superseded granularity) is committed inside 69caff7 whose message names the barrel plan — content/message mismatch disclosed to owner.

- KIND-HOMOGENEOUS OBSERVE: RULED (owner, 2026-07-16, "verdict: adopt") — SUPERSEDES flat signal-only. An observe expression's kind = its triggers' kind; trigger sets must be kind-homogeneous (mixed signal+stream triggers = compile error). Four quadrants: signal->value LEGAL (classic; at most one firing per commit; no chains); stream->stream LEGAL (per-event firing in commit order, one output event per input — the standing per-event expression law wearing structured arms; signals READ inside arms are sampled per the no-implicit-to_stream law, never drive); stream->value BANNED (the crossing is ->latest only); signal->stream BANNED as an implicit drive (compose explicitly: on sig->changes as e). The kind rule becomes TRIGGER-determined, not context-determined; the use context must agree per the stream-result-kind law. Value-land stays chain-free (no running value, no .running, lockstep total). All crossing/retirement rulings unchanged (->changes / ->latest(fallback); to_signal and member-.changes retire; CrossingName closed list). Streams-rework plan §1/§3.A patched accordingly; workflow stopped pre-landing and relaunched.

- BARREL-VISIBILITY PLAN APPROVED (owner, 2026-07-18: "approved barrel visibility plan"). Launches after the policies batch lands, per the locked sequencing.

- MAIN-REMOVAL CONFIRMS (owner, 2026-07-18, discussion ongoing): §3.2 CONFIRMED (suppressible lint dead_top_level_instance; old error class retires). §3.3 CONFIRMED (misapplication = compile error). §3.4 CONFIRMED (zero roots legal = library; runtime declines gracefully "nothing to run"; no lint in a rootless module). §3.6 CONFIRMED (@root precedes the visibility word; question mostly dissolved by barrel — only private survives in-source). §3.8 CONFIRMED (main freed silently; was already ruled via D-05 item 7 + the razor confirmation). ADDITION-2 CONFIRMED: compile-time rules are liveness-blind — types, ownership, uniqueness, orphan, visibility all fire textually on dead placements; liveness governs only runtime existence and the dead-code lint. §3.5 SUPERSEDED BY A GENERAL RULING (owner): ALL applied directives may be written INLINE before their target entity — no @root-specific carve-out; add the general rule if absent (the own-line layout law amends corpus-wide; default pending veto: both layouts legal, own-line AND inline). §3.1 RULED (owner, 2026-07-18: "allow multiple roots") — many @root allowed; live graph = union of root closures; no counting error class. §3.7 CONFIRMED (owner: "clear") — @root is compile-time-only; no IR field; pruned module contains only live instances; runtime stays root-blind. ADDITION-1 REVISED after owner challenge ("what is the point of mention keeping a dead instance alive?"): mention-keeps-alive (option a) WITHDRAWN — it is the glob of liveness (silently grows the live set, mirrors the rejected export-glob) and makes the dead-code lint mushy. Revised recommendation awaiting nod: liveness is TOPOLOGICAL ONLY (the reach-edges are exhaustive); the batch survey verifies every instance-reference form is a reach-edge (expected outcome: vacuously true — pin the one-sentence lemma); if a non-edge reference form surfaces, it comes back to the owner as its own design question rather than defaulting to keep-alive or a silent error class. ADDITION-1 SO CONFIRMED (owner nod, 2026-07-18). MAIN-REMOVAL NOW FULLY SETTLED: all eight §3.x, Phase-0 fold (D-17 root-based vocabulary + D-23 five-member SPEC conform incl. the standing 3-vs-5 divergence), both additions, and the general inline-directive ruling. Remaining gates: plan-doc update with fresh survey AFTER policies + barrel land (they reshape §1.4 / GRAMMAR keyword regions), then explicit owner "approved" before execution. Phase-0 verified on disk 2026-07-18: D-23 half-landed (LOG five-member closure at 021-140/141; SPEC §13.8.1 still three-member — standing LOG↔SPEC divergence, disclosed); D-17 unification never executed (transitive vs containment/interpretation closure vocabularies coexist) — both fold into the main-removal batch.

- POST-POLICIES RULINGS (owner, 2026-07-18): (1) COMMIT MYSTERY RESOLVED — cd1fb5e was the owner's own commit with a wrong message ("I committed with the wrong message"); content verified byte-identical to the policies batch; on record, no history rewrite requested. (2) RIDER-1 RULED, RECOMMENDATION OVERRULED: `is` is comparison (the language's `==` replacement), NEVER assignment/declaration; `type Output = i32` DECLARES — so `=` is the correct binder and the SPEC §3.3 production was right all along. Conform the rest: LOG entries spelling the fulfill binding / trait-side default / own-opt-in with `is` respell to `=` (the associated-type binding family in 005 — binding, default, own form; NOT the where-clause equality predicate); GRAMMAR AssocTypeDecl + AssocTypeBinding productions and the "never `=`" note invert (`=` declares/binds; `is` compares); the ~6 SPEC worked examples `type Item is ...` respell to `=`. Supporting fact: the corpus's own dyn-bracket binding `dyn Iterator[Item = i32]` already uses `=`. PENDING ONE CONFIRM: the where-clause predicate `where Iter.Source is Subject` is a CONSTRAINT/comparison — stays `is` under the owner's razor (derived default, awaiting explicit confirm). (3) RIDER-2 RULED (a): SPEC §13.3.7's at-most-once sentence gains children:/incoming:/outgoing:, conforming to the seven-member list in the LOG's free-order entry. (4) 001 PHILOSOPHY ENTRY REJECTED — the claim was wrong: language constructs ARE the privilege (cell, handle, observe, stream — everything the language provides); the no-privilege doctrine binds LIBRARY code only; language != stdlib. The fold ruling's "result expressible in user code" was fold-specific, not a general law. Proposal withdrawn; the stdlib-side no-privilege facts already live in the topic-local entries; nothing new to write. LEARNINGS #25 strengthened (third incident of the language/stdlib boundary error). (5) SUSPEND PLACEMENT RULED (a): at a gate-closing boundary `suspend` fires and the same-boundary `update` for that instance is SKIPPED — a suspended instance does not reconcile; the gate-open desired re-snap plus resume-then-update covers reconciliation at reopen; amends the just-landed hook-order entries (027-122/031-159 area + SPEC mirrors). (6) 'policy keyword' phrase in the three LOG entries: KEEP (ruled accurate — ring/gate are enumerated clause keywords). ALL of (2)(3)(5) ride the cleanup micro-batch AFTER barrel lands. FURTHER RULINGS (owner, 2026-07-18): fence-tag convention EXTENDS corpus-wide (GRAMMAR.md + IR_GRAMMAR.md fences get the same labels; 002-35's SPEC-scoped wording widens accordingly; rides cleanup). Policies-batch executed wording defaults CONFIRMED (control-flow-and-selector class name; dual section-pointers; 'over ordinary records'; D-28 amend-in-place). `dyn` gets its OWN CLASS per owner (proposal out: type-expression keywords, first member dyn — keywords legal only inside type expressions; placeholder-word fold-in offered as separate check). RIDER-1 BOUNDARY CONFIRMED (owner, 2026-07-18: "yes. confirmed."): the where-clause equality predicate `where Iter.Source is Subject` KEEPS `is` (comparison/constraint); only declaration sites (fulfill binding, trait-side default, own-opt-in) respell to `=`. DYN CLASS RULED (owner, 2026-07-18): option (A) — new keyword class "type-expression keywords", first member `dyn` (keywords legal only inside type expressions); 002-34 amends to assign the class; placeholder-word fold-in (numeric/integer/float/signed/unsigned) stays a separate check, not silently folded. Both ride the cleanup micro-batch. CLEANUP ITEM ADDED (owner "agree", 2026-07-18): 017-9's node-body item enumeration omits `yielded` while 016-1 makes bodies the group's only legal home and 020-8/020-37 list yielded cells and group names in the instance-body scope — reword 017-9 to name yielded declarations among the legal node-body items (found during the conference-pitch exercise; missed conform from the yielded-seventh-keyword amendment). TWO MORE MECHANICAL CLEANUP ITEMS (2026-07-19, no rulings needed): IR_GRAMMAR §2/§6 citation-drift sweep (the 033-166/167/168-area offsets — verify every cited id against current content, refresh stale ones); SPEC §13.19-area example respell `ws.inbound |> count` → `|> event_count` (pre-existing nit flagged at streams-rework, the ruled operator name). PLUS (barrel-batch reviewer catch, pre-existing): GRAMMAR §3.1 TraitDecl comment cites "(003-1 final paragraph)" for a methodless-trait rule — stale under old AND new 003-1; re-anchor to the entry that actually states the marker/methodless-trait law (verify by content). RET-OPERAND RULED (owner, 2026-07-19: "(a). conform the production"): the behavior-IR ret terminator production conforms to the ownership-explicit law — `'ret' operand` replaces `'ret' '%'NAME`, admitting `ret move %v` and literal returns; the move-at-returns prose (033-176) stands as written; no exception is carved into the written-out-never-inferred rule; rides the cleanup micro-batch (IR_GRAMMAR + the 033-210 entry + any SPEC §15.4.4 mirror of the terminator production — fresh-verify each site). PART-2 RESIDUE FOLDED INTO PART-3 SURVEY (owner nod, 2026-07-19): the re-anchor survey's scope widens to the ~80 audit-plan-3 items PLUS the Part-2 residue (duplicate-keyed-by residue, optional strict-kind vocabulary sweep, Portal[WeakHandle[T]] LOG-mirror nicety) — one sweep, one consent-list protocol. With this, Part 2 has ZERO open design points: everything remaining is authorized execution (barrel in flight, cleanup, main-removal after plan-update + "approved", wire-rename), then the widened Part 3 survey. CONNECTION-RENAME RULED IN PRINCIPLE (owner "yes!", 2026-07-18): the connection construct renames, with three pins LOCKED — (1) full uniformity: declaration keyword, intrinsic marker trait, IR primitive, and connection-view vocabulary all rename together (no surface/IR divergence); (2) the wiring-type two-level vocabulary keeps its name, disposition (a): one disambiguating LOG sentence pins the two senses; (3) sequencing: own amendment AFTER main-removal, BEFORE the Part 3 re-anchor survey. NAME PENDING: wire vs link — owner leans wire; head-to-head comparison requested and delivered; assistant recommendation: wire (flow connotation matches the construct's job; corpus already speaks circuit register — wiring types, wire-candidate envelopes, 'wired into the reactive graph'; link collides with linker/toolchain vocabulary in unrelated senses, violating the one-word-one-sense razor from the export-vs-expose ruling; directed-wire is idiomatic in the signal-flow/patch-cable register; choosing wire also keeps Link free as a user identifier, which 022-75's own example shows is natural). NAME RULED (owner, 2026-07-18): "wire." The construct renames connection -> wire under the three locked pins: declaration keyword `wire`, intrinsic marker trait `Wire`, IR primitive `wire`, connection-views -> wire-views; wiring-type vocabulary kept with one disambiguating LOG sentence (a wire is a nominal graph entity annotating by type name, not a wiring type; the two senses are kin, not collision); own amendment (WIRE-RENAME) sequenced after main-removal and before the Part 3 re-anchor survey; blast radius: section 019 wholesale, 002-3 keyword list, 005-183 marker triple, sealed-kind list, SPEC section 13.6 chapter + connection prose corpus-wide, GRAMMAR productions/keyword box, IR_GRAMMAR six-primitives + connection entries; the 022-75 example type Link stays available to users; full protocol applies (survey -> plan doc -> approval -> execution -> verify chain). MAIN-REMOVAL PLAN APPROVED (owner, 2026-07-19: "approved") — no vetoes; the three §7 defaults stand (Phase-0 fold; "root closure" canonical with interpretation/containment closure distinct; both directive layouts legal + inline stacking). FINISH-LINE AUTHORIZATION (owner, 2026-07-19, verbatim intent): "now you have everything to finalize Part 2. if something genuinely conflicting arises - do disclose it for discussion/approval, but otherwise, if no such thing arises that requires ruling - drive the Part 2 to its finish line respecting the orchestration protocol and provide the final report afterwards." — So: cleanup → main-removal → wire-rename+channel amendment proceed WITHOUT further per-batch approval gates; the wire-rename plan doc is still written as a protocol artifact (survey-grounded, compaction insurance) but execution is not gated on a separate "approved"; genuine conflicts halt and return to the owner; one consolidated final report closes Part 2. PIN (2) SUPERSEDED — CHANNEL VOCABULARY RULED (owner, 2026-07-19: "go with channels. settled."): the "wiring type" class term RETIRES; the applied kind annotations (cell T, signal T, derived T, recurrent[N] T, stream[P] T, yielded T, dynamic view T) are CHANNELS — a prose/LOG term, NOT a keyword (nothing reserved; users keep Channel as an identifier, unlike wire). Two flavors: VALUE CHANNELS (the cell umbrella — deliver the latest committed value, snapshot-readable) and EVENT CHANNELS (streams — deliver each event once per cursor); yielded = a membership channel, dynamic view = a reference channel. The class definition keeps type-system membership per the two-level ruling: 'a channel — a type-system member, unstorable by nature: a mechanism that delivers values reactively, never a value.' 016-286's host-written "two channels" is PROMOTED to an instance of the general class (one value-channel form, one event-channel form), not reframed. The crossings razor respells: the arrows mint between channel shapes (sig->changes mints an event channel from a value channel; ->latest the reverse); informal "wiring" prose audited site-by-site in the same amendment. Candidates rejected on the way: kind type (vague + type-as-data), conduit/duct (flow-biased), feed (precise but obscure); channel carries external precedent (tokio watch = state channel; SystemC sc_signal = a primitive channel; .NET DropOldest/Wait ≈ ring/gate) and internal precedent (016-286). Rides the WIRE-RENAME amendment — the amendment now carries BOTH renames: connection→wire and wiring-type→channel. PRELIMINARY CHANNEL-SIDE SITE LIST (floor, not ceiling — the amendment's survey completes the class against the then-current corpus): LOG wiring-type family 016-62/163/166/178/180, 017-189 (never-nests law), 030-36 ('a stream is wiring, not a value'), 030-47, 030-51 (umbrella sentence); 016-286 promotion; crossings-razor sites 030-269..272; DOUBLE-HIT 017-167 ('persistent wiring between instances is a connection' — both renames land in one entry); informal-prose audit sites 013-4/013-247 (category-C 'reactive wiring'), 021-16/17 (placement wiring), 029-5 ('wired into the reactive graph'); SPEC mirrors §13.2.8, §13.18.3, §13.18.5, §13.18.9 razor prose; GRAMMAR KindAnnotation production comments. 'wire-candidate envelope' STAYS (becomes literal).

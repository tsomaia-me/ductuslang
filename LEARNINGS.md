...
25. Never say "stdlib" for language-builtin concepts: streams, their policy machinery (Ring/Gate and the policy-parameterized stream family), Map, and the other language-provided primitives are LANGUAGE builtins — lower level than any library. "stdlib" means the separate, external library layered on top (D-29's distinction: Map is language-provided; Vec is borrowed from the stdlib's future spec). Calling a language builtin "stdlib" is a category error in both prose and LOG/SPEC text. Example: describing RingStream[T, N] as "stdlib sugar" — wrong; and LOG 030-261's own "The stdlib provides transparent generic aliases…" wording carries the same defect. Second incident: calling a language crossing member "one-line stdlib sugar" — doubly wrong: the crossing is language-level, and "sugar" is itself a language concept the stdlib cannot define; stdlib is external, unprivileged, plain code.

26. Ruled-item re-presentation is a veto window and misrecording insurance — keep it, but terse: items whose defaults the owner already explicitly confirmed get a one-line "executes as ruled; default X stands unless vetoed" notice; the full background/problem/options treatment is reserved for unruled questions, never-confirmed defaults, and rulings recorded under compression risk (the D-18 "auto-conversion" misrecording is why the insurance exists). For bulk decision sets (Part 3's ~80 items): classify dissolved/executed/stale/live first; live items go out in themed consent-lists of one-liners (problem → recommendation → why) that the owner approves wholesale and strikes to escalate — only struck items get full elaboration. Never present a verify-only step (one grep) as a batch item; it belongs in the gate list.

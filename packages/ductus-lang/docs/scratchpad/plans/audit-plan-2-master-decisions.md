...
- D-02: RULED — adopt the SPEC's split (six declaration kinds; distinct cell-kind umbrella). Owner Q on recurrent stream answered: it is the stream declaration kind carrying the recurrent history modifier — named explicitly in the canonical membership line, not a seventh declaration kind. OPEN residual: yielded-as-operator-parameter (P026) — cross-section design call, not executed.
...
- D-08: RULED — Model B placement (all five hooks post-publish inside blocking commit()) + per-instance order teardown, create, resume, update. Residuals OPEN: F089 initial-cohort moment; F108 reload-commit tail.
...
- D-12: RULED — split: observed cells do NOT block the satisfies-waiver; they DO block auto-satisfaction. Residual OPEN: F238 (does a default body exposing the observed cells restore auto-satisfaction?).
...
- D-20: RULED — Model B (match selects/unrolls; never freezes arms). Residuals OPEN: whether the surviving unroll/tag lowering gets SPEC elaboration; interpretation-context term fate.
- D-21: RULED — Model B (member order as pivot; walk order and declared order as the per-source definitions). Walk-order homed in section 034 per recommendation; walk order explicitly distinct from exposition entry order.
- D-22: RULED — merged into D-18's stream ruling: streams (regular + recurrent) form one separate kind class. OPEN flag: whether yielded T stays in the cell umbrella (it also lacks a current value) — owner to confirm.
...
- D-26: RULED — Model A (per-declaration desired cells). Residual defaults per item recommendation (desired streams reuse the one stream cell form; whole-record kept as reconciler-facing view; per-arm gates with narrowed no-activation-input claim).
...
- D-30: OPEN — not ruled (effect-instance identity across reload).
- D-31: PROPOSED, awaiting owner word (normative status of GRAMMAR.md / IR_GRAMMAR.md vs 001-3/002-29).
...
- NEW OPEN (from D-15's grounding): D-10 tension — 'yielded fulfills Iterable' has the same membership-varying hole; owner to rule whether groups keep D-10's reactive-re-run carve-out or lose Iterable like views did.

...
> **For execution:** Canonical = model B (lowercase kind `cell T`; no `Cell[T]`/`Signal[T]`/`Derived[T]`/`Recurrent[T,N]`/`Stream[T]` types). SPEC basis unchanged and authoritative: SPEC.md §13.2.8 lines 12377-12386 (kind, never bracketed), 12439-12441 ("no `Cell[T]` type"), 12508-12512 (taxonomy incl. `Portal[cell T]`, `dynamic view V` replaces bracket), 20545-20551 (older type-vs-trait framing "neither is correct"); operator kind usage §13.17 (e.g. SPEC.md:19767 `cell T`≅`T`). Conform these LOG ids to the kind spelling: 013-10; 016-62, 016-63, 016-165, 016-167, 016-171, 016-172, 016-175, 016-179, 016-180, 016-246, 016-266, 016-270, 016-279, 016-280, 016-281, 016-282, 016-283; 017-23, 017-53, 017-71, 017-125, 017-193, 017-194, 017-215; 025-13, 025-59; 029-2, 029-6, 029-11, 029-12, 029-18, 029-20, 029-22, 029-31, 029-41, 029-45, 029-69, 029-75, 029-112, 029-122, 029-124; 030-48; 031-19, 031-25. Recurrent depth spelling `Recurrent[T,N]` -> `recurrent[N] T` (F158: 029-11, 029-69). Portal carrier `Portal[Cell[T]]` -> `Portal[cell T]`, `Option[&Cell[T]]` -> `Option[&cell T]` (F195/F159/F151: 016-179, 017-215). Findings resolved: F194, F195, F196, F197, F198, F183, F184, F187, F221, F059, F061, F146, F148, F150, F156, F158, F159, F149 auto; STILL residuals in F062 (delete-vs-reword bracket entries in §016), F147 (first-class-type entry 016-167 reword/drop), F157 (029-45 citation repoint to operator-output §13.17.x). Verification greps: after edits, `grep -nE 'Cell\[|Signal\[|Derived\[|Recurrent\[|Stream\[' DECISION_LOG.md` should return only intentional non-cell brackets (e.g. `Handle[T]`, `Portal[...]`, `Type[C]`, `Map[K,V]`, `WeakHandle`); `grep -n 'Portal\[Cell\[T\]\]\|Cell\[DynamicView' DECISION_LOG.md` should return zero; confirm SPEC still says `grep -n 'no .Cell\[T\]. type' SPEC.md` (line 12441).
...
> **For execution:** Canonical side = SPEC's six-declaration-kinds + distinct cell-kind umbrella (SPEC §13.2 header at SPEC.md:11550-11556; §13.2.8 cell umbrella; §13.20.4 at SPEC.md:23042-23047 makes `yielded` a cell-KIND). Depends on K-CELL-MODEL (must resolve kind-vs-bracket first). LOG to conform: 016-1 (DECISION_LOG.md:1854 — re-scope "seven reactive declaration kinds" header; separate the declaration-keyword row from the cell-kind row; the entry already flags "two rows … not the same list"); 016-162 (:2015 — "six declaration kinds", align cardinal + members); 016-156 (:2009 — five-name enumeration, reconcile with the six); 016-4/016-115/016-118 (:1857/:1968/:1971 — const is a declaration kind but not a cell kind, keep non-reactive); 016-62/016-163/016-166 (:1915/:2016/:2019 — collapse `cell T` umbrella to one membership list, include yielded, decide attr spelling); 013-199 (:1585 — broaden to "any reactive cell kind" or cross-ref 025-55 at :3152); 029-12 (:3391) / 034-11 (:4362) — resolve yielded-as-operator-param (add fourth param kind vs narrow consumer surface). Findings: F153 (H, main report L32), F155 (M, L397), F154 (M, L413), F066 (L, L520), F049 (L, L485 — STILL), P026 (M, addendum L490 — STILL). Verify: `grep -n 'seven reactive declaration\|six declaration kinds\|all six declaration' DECISION_LOG.md SPEC.md` should return one consistent story; `grep -n '`cell T` is the umbrella' DECISION_LOG.md` should yield one authoritative membership; confirm no LOG entry lists `const` among cell-umbrella members and no entry omits `yielded` from it.
...
> **For execution:** Canonical side = five-kind set {`signal`, `derived`, `recurrent`, `stream`, `const`}; no module-level `let`, no module-level `attr`. Conform these entries to state it identically: 013-22 (LOG line 1408, keep 'module scope contains no let bindings'), 013-23 (line 1409, change to `const, signal, derived, recurrent, stream`; drop `attr`, add `stream`), 013-30 (line 1416, strike 'top-level `let`' - keep `let, mut, const, signal`), 020-9 (line 2707, drop `let`; result `signal, derived, recurrent, stream, const`). SPEC: section 11.2 (SPEC.md lines 8186 and 8210-8213 - strike the 'top-level `let`' phrase at 8186, correct the module-scope enumeration to add `stream` and drop `attr`), section 13.7.1 (SPEC.md lines 16191-16192 - drop `let` from the module-top-level list). Supporting facts: attr definition 016-14 (line 1867, per-instance/placement-written); module-scope stream 027-89 (line 3267); mut-forbidden-at-module-top-level 013-19 (line 1405). Findings: P007 (HIGH, module-level let contradiction), P008 (MED, enumeration drift). Depends-on: K-KIND-COUNT (cell-kind set). Verify: `grep -n 'module scope contains\|module top-level scope comprises\|Module scope contains' DECISION_LOG.md` should show one identical five-kind set; `grep -n 'top-level .let.\|module-level .*let' DECISION_LOG.md SPEC.md` should return zero module-level-let mentions.
...
> **For execution:** Canonical side after this decision = the fold-as-derived IR model (Route 1) + Model B group + derived liveness (L1) + leave-is-a-dirty-root. DEPENDS ON: K-KIND-COUNT (kind enum + count), K-CELL-MODEL (cell = kind vs value). LOG to conform: 034-9 (4360: strike "on/off bits"/"reuses gate on/off-bit machinery"; restate liveness as derived effective-activation), 034-10 (4361: keep "propagates dirt"; drop "distinct cell-KIND" implying a 5th enum tag under Model B), 034-11 (4362) + 018-37 (2507) + 018-41 (2511): widen repeat-source rule to admit a yielded group as a third category; 034-12 (4363: Iterable-runtime-only unchanged), 034-13 (4364: `.count` = synthesized cell), 034-3 (4354: membership drivers unchanged). 033-58 (4170) + 033-59 (4171): add yielded/collect lowering row + fold-kind assignment trigger (a `derived` whose RHS is a `fold` carries kind=fold). 033-80 (4192) + 035-9 (4376): keep enum `input|derived|recurrent|fold`; add rule that a fold cell is scheduled as a derived DAG node. 033-81 (4193) + 033-82 (4194) + 035-10 (4377): fold payload unchanged; state member edges feed the derived dependency-edge list. 033-169 (4281): add `fold` to the kind-led text-form enumeration. 023-14 (3022): amend DAG-node enumeration to include fold cells (as derived nodes) OR state fold is scheduled-as-derived. 023-8/9/10/12 (3016–3020): add membership-change dirty-root rule tied to the gate transition / repeat re-key, respecting no-new-dirty-bits-after-step-1. 024-2 (3069) + 024-4 (3071): add fold combiner/member reads and reads-of-fold-cell to the cycle-graph walk. 035-5 (4372): cost rule unchanged. SPEC to conform: §13.20 / §13.20.4 (23044–23090: strike on/off-bit prose, define standalone-group IR object, define `.count` cell), §13.21.7 (23183–23195: state fold scheduled-as-derived), §15.4.1 (24044–24069 fold payload; 24086–24091 add member edges to derived edge-list; 24512/24540–24549 text-form already lists fold — reconcile with LOG 033-169), §13.10.2 (18127–18148: add membership dirty-root + fold DAG node), §13.9.8 (17818–17836: narrow "Gate-close adds no DAG work" to exclude folds with a gate-guarded member), §13.9.7 (17805–17816 activation-driven membership), §13.11.1 (18314–18319: fold reads in cycle graph), §13.5.4.1 (15265–15279: admit yielded repeat source). Findings: F114, F123, F093, F248, F247, F246, F213, F211, F171, F115, F167, F124, F096, F095, F094, F097, F164, F242, F099, F098. Verification greps: `grep -n 'on/off bit' SPEC.md DECISION_LOG.md` must return 0 after edit; `grep -n 'input | derived | recurrent | fold\|input|derived|recurrent|fold' DECISION_LOG.md` confirms enum unchanged; `grep -n 'repeat.*source\|Signal\[I\]' DECISION_LOG.md` at 018-37 must name the yielded category; `grep -n 'fold' DECISION_LOG.md` at 023-14 and 024-2/4 must show fold in DAG/cycle rules.
...
> - **Auto-conform (3):** F012 (LOG 005-68/005-69/005-70 @ DECISION_LOG.md:409-411, cite §3.1.1) — validated, no edit; §3.1.1.1 nests at SPEC.md:1223 between §3.1.1 (1192) and §3.1.2 (1255). F040 (LOG 008-57 @ :917) — repoint (§5.7)→(§13.2.8) to match sibling 016-179 @ :2032; razor text at SPEC.md:12515. F036 (LOG 007-234/007-235 @ :852-853) — repoint (§4.9.4)→(§4.9.5) to match sibling 007-237 @ :855; Hash table at SPEC.md:4263-4270.
> - **STILL (6):** F185 (LOG 017-190 @ :2331; §8.4.1 @ SPEC.md:6136; optional-chaining elaboration at §13.3.6.2 region, SPEC.md:14162/14321-14324). F034 (LOG 006-27 @ :609; §3.5.7 @ SPEC.md:2336-2348; carve-out at §13.3.7.7, SPEC.md:14798). F016 (LOG 009-91 @ :1028; §6.2.4 @ SPEC.md:5254-5316; ban at §13.3.7.7, SPEC.md:14808-14810 — back-cites §6.2.4). F207 (LOG 033-113 @ :4225; §15.4.6 @ SPEC.md:24485-24561, zero "Portal" hits; real elaboration §13.3.6.3 @ SPEC.md:14342/14419). F175 (LOG 021-141 @ :2880; §13.14.1 @ SPEC.md:18885; no owning section identified). F174 (LOG 021-16 @ :2755; §13.8.2.1 @ SPEC.md:16521-16542, attr-RHS only).
> - **Verification greps:** nesting check `grep -n '^#### 3\.1\.1 \|^##### 3\.1\.1\.1 \|^#### 3\.1\.2 ' SPEC.md`; per-repoint, confirm target string exists, e.g. `grep -n 'storability razor' SPEC.md` (§13.2.8, expect 12515), `grep -n 'Built-in Hash conformance' SPEC.md` (§4.9.5), `grep -n 'A portal preserves across reload' SPEC.md` (§13.3.6.3). After each repoint: re-grep the cited section body for the claim's keyword to confirm the new pointer lands.
...
> **For execution:** Canonical side is the LOG's Invariant 2 (DECISION_LOG.md line 11): restate locally only when an entry is "incomplete without another rule's content." Apply the rider test per family. AUTOMATIC collapses (verbatim, no rider): F132 — merge the four-slot borrow-storage list, keep in 001-25 (line 55), drop the duplicate clause from 013-52 (line 1438) or scope 013-52 to the alias-specific case; F131 — consolidate the "no separate grammar document / single authoritative source" fact across 001-3, 001-4 (lines 33-34), 002-29 (line 100), partial 001-1 (line 31); F092+F190+F260 (one defect) — merge adjacent 027-80 and 027-81 (lines 3258-3259, §13.14.7) into one entry preserving both the generic-instantiation refusal and the general startup-refuse-with-diagnostic clause. STILL families (surface, user picks keep-vs-trim): F135 — 005-185 (line 526), 016-223 (line 2076), 017-167 (line 2308), 019-21 (line 2637), 031-6 (line 3775); SPEC §3.7.4, 13.2.10, 13.3.6.1, 13.6.0, 13.19. F133 — 016-224 (line 2077), 017-170 (line 2311), 031-5 (line 3774); SPEC §13.2.10, 13.3.6.1, 13.19. F134 — 006-27 (line 609), 009-91 (line 1028), 017-221 (line 2362); SPEC §3.5.7, 6.2.4, 13.3.7. F092 nine-site: 027-80, 027-81, 031-119, 031-128, 031-143, 031-157, 033-124, plus 015-39 (line 1848, the ", and" punctuation outlier flagged separately as F191) and SPEC §13.14.7, 13.19.14 — recommend leave-as-required-restatement, optionally normalize punctuation. Constraint: Invariant 1 dense positional numbering — every deletion renumbers later entries in the same section; do not apply collapses silently. Verification greps: `grep -n 'record field, tuple component, enum payload, or indexed slot' DECISION_LOG.md` (expect 001-25, 013-52); `grep -n 'no separate grammar document' DECISION_LOG.md` (expect 001-3, 002-29); `grep -n 'required if and only if' DECISION_LOG.md` (expect the ~7 reconciler sites); `grep -n 'carried .*via .*Type\[' DECISION_LOG.md`. Findings: F131, F132, F133, F134, F135, F092/F190/F260.
...
> - **F188** (contradiction, placement): entries in conflict are 027-50 + SPEC §13.14.4 (SPEC.md ~18988-18991, Model A: suspend/resume before publish) vs 027-32, 027-91, 027-96 + SPEC §13.14.9 (SPEC.md ~19174-19176, Model B: after publish). To adopt Model B: rewrite 027-50 so `commit` does not itself fire suspend/resume before publish, and remove the "fires suspend/resume ... and publishes" clause from SPEC §13.14.4; keep 027-91/027-96 as the single source of truth. Conform SPEC §13.14.4 to §13.14.9.
> - **F091** (create-before-update): add an ordering rule near 027-69/027-70 (§13.14.7) and 027-91/027-96 (§13.14.9); reconcile with the update-enumeration 031-124 (§13.19.14, DECISION_LOG.md:3893) which currently lacks the "existing" qualifier that 027-70 carries.
> - **F089** (initial-cohort create): add a startup step in the 027-16..027-28 sequence (§13.14.1, DECISION_LOG.md:3194-3206) pinning when the initial cohort's `create` fires relative to 027-28 first-commit and 027-29 live-transition.
> - **F249** (resume-before-update at gate-open): add ordering in §13.14.9 or §13.19.14; anchors 022-59 (§13.9.7, DECISION_LOG.md:2943), 031-108 (§13.19.x), 031-124/125 (§13.19.14).
> - **F108** (reload commit tail): state in 028-23 / SPEC §13.15.3 (SPEC.md:19353, "9. Commit the reloaded state") whether step 9 runs the 031-124/125 reconciler tail or defers; note 027-32's "in steady state" qualifier is why the universal rule does not auto-apply.
> - **F189** (blocking): no normative change required (verifier refuted the ambiguity via 027-32 + 027-52, DECISION_LOG.md:3210 and 3230); optionally add one SPEC §13.14.9 sentence stating `commit()` blocks until hooks return.
...
> **For execution:** Canonical side = Model A, the settled §13.3.6.2 split (LOG 017-176, 017-179, 017-181, 017-182, 017-186: `Handle[T]` static / auto-derefs `&T` / never freezes; `WeakHandle[T]` dynamic / reads `Option[&T]` / freezes on `None`). AUTO fixes (relabel `Handle`→`WeakHandle`, SPEC-conforms-to-LOG): F228 [H] SPEC §13.8.5.1 lines ~16959-16963 → `WeakHandle[N]` (match LOG 021-82/021-83, 019-52/019-53); F172 [M] SPEC §13.8.4 lines ~16808-16811 and §13.8.5.1 lines ~16961-16963 → `WeakHandle[N]` (match LOG 021-56, 021-82, 021-83); F177 [M] LOG 019-49 (line 2665) → name only a re-pointing kind (`WeakHandle`/reactive selection), reconcile with 019-52 (2668), 019-17 (2633), SPEC §13.6.2 line ~16024 which already says `WeakHandle`; F176 [L] LOG 021-57 (line 2796) → `WeakHandle` (match 021-56/2795, 021-82/2821, 021-83/2822, 021-84/2823); F085 [M] SPEC §13.3.4.2 worked example lines ~14019-14028 → type `wheel: WeakHandle[Drivable]` (match LOG 017-139/2280, 017-141/2282). STILL: P020 [H] LOG 017-111 (line 2252) + 017-147 (line 2288) + SPEC §13.3.4 lines ~13895-13898 → user picks mislabel (→`WeakHandle`) vs umbrella (add carve-out that only `WeakHandle` dynamizes membership; static `Handle[T]` keeps static `incoming`); note SPEC 13897 self-refutingly cites §13.6.2 which per 019-52/53 makes `Handle[N]` static. P021 [M] LOG 017-136 (line 2277) third form → user picks relabel-to-`WeakHandle` vs admit static `Handle[T]` self-sourced form + add its static-membership entry; elaboration 017-139/140/141 (lines 2280-2282) already says `WeakHandle`. VERIFY: `grep -n 'Handle' packages/ductus-lang/docs/SPEC.md` around §13.8.4/§13.8.5.1/§13.6.2/§13.3.4/§13.3.4.2 shows no bracketed `Handle[N]` in an `Option[&N]`/freeze/re-point/dynamic-membership context; `grep -n '^019-49\.\|^021-57\.\|^017-111\.\|^017-136\.\|^017-147\.' packages/ductus-lang/docs/DECISION_LOG.md` shows the dynamic role spelled `WeakHandle` (or, for P020/P021, the chosen umbrella carve-out present).
...
> **For execution:** Canonical side = the amendment's LOG (034-9, 034-10, 034-11, 034-12; §13.20.4.2). Conform SPEC §13.20.4.2 (SPEC ~23082-23090) to Model 2 by adding two clauses: (1) `Iter.Source = Subject` where Subject is the `yielded` group's on/off-bit view (a third, language-only Source pattern, neither `type Source = ()` per SPEC ~10737 nor `type Source = TheCollection` per SPEC ~10741), item is by-value/Copy so it is rooted nowhere (dissolving 014-29's borrow-equivalent-rooted default form for this fulfillment, LOG 1668); (2) a member join/leave is a reactive membership update per 034-10 (LOG 4361), so the in-progress pass is atomic per 014-161 (LOG 1800) and the loop re-runs next cycle per §12.13 (SPEC ~11295-11299) — reconcile with the source-freeze invariant §12.8.2 (SPEC ~10920-10923) by scoping freeze to within-a-pass. Resolves F051, F050 automatically; F044 [PLAUSIBLE] is the ch12-side duplicate — dismiss if clauses (1)+(2) land. F052 [STILL]: replace "runtime-loop contexts" in 034-12 (LOG 4363) and §13.20.4.2 (SPEC 23087) with the runtime-vs-compile-time-unrolled axis of 014-58 (LOG 1697). Depends on K-YIELDFOLD-IR (fold/yielded IR serialization, 033-80/035-9/035-10). Verify: `grep -n 'runtime-loop context' DECISION_LOG.md SPEC.md` should return zero after F052 fix; `grep -n 'Source = Subject\|language-defined fulfillment' SPEC.md` should show the new by-value Source clause at §13.20.4.2.
...
> **For execution:** canonical side = Option A (bare fence = Ductus source; non-Ductus fences carry an info string that excludes them from Ductus lex checks). Add a new decision under topic 002 (lexing) stating the convention: bare fence info-string = Ductus source; a non-empty info string marks a non-Ductus block excluded from Ductus lexing rules including the semicolon-is-lex-error rule of LOG 002-16 (§1.4). Update SPEC §1.4 "Conventions" (line 115) to add this — its current sentence "Code examples use Ductus syntax." (line 117) is the contradicting text and must be amended to admit tagged non-Ductus blocks. Then tag the flagged fences: SPEC ~24437/24493 (IR module + EBNF grammar, evidence at 24459, 24530); ~1118/17905/21402 (diagnostic hint prose, evidence 1126, 17910, 21406); ~11523/19008 (host loop/transaction pseudocode, evidence 11524-11526, 19010-19011); ~18111/18221 (host write-signal pseudocode, evidence 18111-18115, 18221-18225). Findings: F145, F144, F143, F142 (all LOW / stale_example / CONFIRMED; anchor LOG 002-16). Verify: `grep -c '^```$' SPEC.md` should drop as bare non-Ductus fences get tagged; `grep -n '^```$' SPEC.md` remaining bare fences should all contain valid Ductus (no `;` outside comments/strings); confirm no bare fence contains a raw `;`.
...
> **For execution:** Canonical side = LOG. Findings resolved: F237, F014, F010 (auto); F238 (still — residual on default-body-exposes-observed). Waiver: LOG 005-67 (waiver text) and 005-71 (mandatory-satisfies text) keep "required cell" in its node/connection sense per 005-49/005-50/005-59 (§3.1.7); edit SPEC §3.3.5 lines 1974-1985 to REMOVE the clause folding "`observed:` contract cells that a fulfillment must supply" into the waiver-blocking "no required cells" set (SPEC.md:1976-1978), leaving only the node/connection §3.1.7 members as blockers. Optionally add a one-line LOG note to 005-67 clarifying observed cells (005-69, "required output cells") do NOT count for the waiver. Auto-satisfaction: amend LOG 005-109 (§3.3.5) to add the observed-contract bar per the residual answer; mirror into SPEC §3.3.5 auto-satisfaction prose (SPEC.md:1944-1954), which is currently silent. Related but out of scope for this item: F013 (effect-method + node-cell jointly-unsatisfiable trait), F011 (first-param Subject modality), F015 (undefined "methodless-trait waiver"), F069/F070 (bootstrap `|>` Case-3) — do not resolve here. Verify: `grep -n 'observed:.*contract cells\|no required cells' packages/ductus-lang/docs/SPEC.md`; `grep -n '^005-67\.\|^005-71\.\|^005-109\.\|^005-69\.' packages/ductus-lang/docs/DECISION_LOG.md`; confirm the term "required cell" resolves to §3.1.7 node/connection members only in every waiver/auto-sat use.
...
> **For execution:** Canonical side = Model B (effect methods excluded from ordinary dispatch). Conform these LOG entries: 005-78 (effective-method-set — qualify "enter exactly as value methods do" to mean set-membership for collision only, not dispatch candidacy), 005-114 (three call forms — carve out effect methods, value methods only), 005-120 and 005-121 (bare-name/method-call trait-impl search — add effect-method exclusion). SPEC: add an explicit exclusion/diagnostic step in §3.4.1 (currently lines 2038-2078, step 1 at 2038-2042, step 4 at 2057-2064); keep §3.1.1.1 (lines 1250-1253) as the intent statement and cross-reference it from §3.4.1; reconcile the collision-namespace bullet §3.1.1.1 lines 1245-1249 to say "collision namespace only, not a dispatch candidate." Findings: F068 (gap, auto-resolves), F033 (divergence), F067 (contradiction; drop its 017-273 citation per the charity note). Verify: grep -n 'effect' SPEC.md §3.4/§3.4.1 (lines 2000-2145) and DECISION_LOG 005-120..137 must now be non-empty; grep the new exclusion clause; confirm 005-114 carve-out text exists.
...
> **For execution:** Canonical side = the wide Map key set; `StringifiableKey` must be widened to match it. LOG to amend: **018-64** (widen `StringifiableKey` enumeration from `i8`–`i64`, `u8`–`u64`, `bool`, `char`, `string` to `i8`–`i128`, `u8`–`u128`, `isize`, `usize`, `bool`, `char`, `string`, `duration`, `instant`); verify **018-63** (keyed-by result bound), **018-69** (`Keyed::Key: StringifiableKey`), **018-71** (stringifiable-element path) still read correctly against the widened set; verify **012-101** (Map key bound — LOG says "integers" broadly) and reconcile its wording with the widened `StringifiableKey` and with the SPEC list. Cross-check no-op cases: **018-42** (stream→Map→repeat canonical path), **018-58/018-127/018-142** (`at <index>` usize + `keyed by <index>` positional remedy), **018-44** (Map yields `(K,V)`), **018-83** (Map diffed by key identity — F166). SPEC to conform: **§13.5.4.1** (line ~15352, the `StringifiableKey` enumeration in the key-derivation precedence list) — must match amended 018-64; **§9.5.3** (line ~7311, Map key-type list) — cross-reference for the canonical set; **§15.4.1.1 / §13.5.4** path-component rationale (line ~15218) confirms the stringifiability rationale that grounds Model A. Findings: F229 [H], F163 [H], F166 [L/auto]. Verification greps: `grep -n 'StringifiableKey' SPEC.md` (all 8 sites must agree on the widened set), `grep -n '^018-6[3-4]\.\|^012-101\.' DECISION_LOG.md`, and confirm no coercion/widening rule was silently added — the fix is enumeration-widening, not a cast rule.
...
> **For execution:** Canonical side = LOG (SPEC conforms). Adopt Model B. (1) Amend the general iteration rules — LOG 014-2 (line 1641), 014-125 (1764), 014-135 (1774), and 014-3 (1642) — to add an explicit carve-out: a language-level kind may satisfy `Iterable`/`IntoIterable` for consumption by operators/`repeat` while rejecting user-surface `for`/`for own`; keep 017-81 (2222) as that exception's instance. Cross-check 014-141 (1780: no stdlib privilege) — the carve-out is a language-kind exception, not a stdlib one, so 014-141 stays intact. (2) Retire the false rationale in SPEC §13.3.3.4 (lines 13614–13620, "`for` is compile-time, §12.3.7…"); replace with the weak-handle/no-positional-identity/supplier-key reasons already at 13617–13620; this closes F256 against 014-58 (1697)/014-65 (1704) and SPEC §12.3.7 (10315–10318). (3) Resolve F257: decide `IntoIterable` status for the view kind at 017-192 (2333) and SPEC §13.3.3.4 (13641), reconciling with 014-127 (1766)/014-130 (1769)/014-119 (1758); state `for own`'s behavior or drop the `IntoIterable` claim. Findings: F255 [H], F256 [M, auto], F257 [L]. Verify: `grep -n '^017-192\.\|^017-81\.\|^014-125\.' DECISION_LOG.md`; confirm SPEC §13.3.3.4 no longer cites "`for` is compile-time"; confirm the auto-dispatch entries carry the carve-out clause.
...
> **For execution:** Canonical side: model A (zero rows). LOG entries to conform: 017-89 (line 2230) — reword "an offset table of length 1 holding a single zero-length row" to "an empty offsets table of length 1, encoding zero rows (offsets length = rows + 1)"; 017-90 (line 2231) — add that `bundle.count` is the row count as an intentional exception to the element-tally unification. Resolve F086's residual by amending 012-91 (line 1286) to carve bundles out of "reports the number of elements" (bundle `.count` = rows). SPEC §13.3.3.5: lines 13678-13680 (reword "single zero-length row" to zero rows; keep "zero elements"); lines 13692-13693 (mark `bundle.count` = row count as the element-tally exception); lines 13708-13715 (state offsets-length = rows + 1 invariant explicitly, so length 1 = zero rows). Findings: F086, F057, F162. Verify: `grep -n 'zero-length row' SPEC.md DECISION_LOG.md` returns nothing after edit; `grep -n 'rows + 1\|offsets.*length' SPEC.md` finds the new invariant; confirm 012-91 no longer lists bundles under "number of elements" unqualified.
...
> **For execution:** Canonical side = Model A (read-site exclusion), Model B does not apply because §13.18.7.3 requires a `stream`/`recurrent[N] stream` surrounding declaration. LOG entries that already state Model A and stand as-is: 029-124 (line 3503, §13.17.3), 016-283 (line 2136). Model B entries that stay scoped to `stream`-declaration contexts and do NOT need change (they already carry the surrounding-declaration constraint via 030-82): 030-64 (3570), 030-65 (3571), 030-66 (3572), 030-82 (3588, §13.18.7.3). 029-98 (3477, single-commit) unaffected. SPEC: read-site-exclusion text at §13.18.5 (SPEC 20555-20557) and the value-cell-kinds list (SPEC 12415-12418); per-event model at §13.18.7.1 (SPEC 20624-20628); surrounding-declaration rule at §13.18.7.3; existing diagnostic at SPEC 21743-21752; operator-diagnostics enumeration at §13.17.12 (SPEC 20140). Fix for F128: add/point a concrete diagnostic for reading a stream-bound `cell T` param as a value in an operator body (recommend reuse of SPEC 21746 "cannot read a `stream T` as a value" + cross-ref from §13.18.5 / 029-124), OR add a §13.17.12 class. Findings: F128 (MED/divergence/CONFIRMED) — pin mechanism, stays open pending F128 residual (a)/(b). F129 (MED/gap/PLAUSIBLE) — dismiss under Model A (construct rejected upstream). Verification greps: `grep -n 'excluded at the read site' SPEC.md` (expect 3 hits: ~20556, ~12417, LOG 029-124); `grep -n 'cannot read a `stream T` as a value' SPEC.md`; `grep -n 'surrounding declaration must be' SPEC.md` to confirm §13.18.7.3 constraint intact. Corpus pin: HEAD 1b0c6b8.
...
> **For execution:** Canonical side = DRAIN + downstream-only "complete". Add a new LOG decision in §13.18.12 (consumer cursors) stating that a logically-completed subset consumer (`take`, and any operator that stops emitting while its source lives) continues to advance its source cursor and releases slowest-cursor retention — mirroring the `@reset_on_reopen` gate release (030-196) but automatic and permanent, and explicitly distinct from the gate-off freeze trigger (030-191/193, SPEC §13.18.12 lines 21484–21498). Add a companion LOG decision in §13.18.9 defining stream "complete" as a downstream-emission property only, with the three F201 sub-answers: merge arm completion (relates 030-143), derived-consumer completion (relates 030-130/137/139/140/149), observation-cell non-surfacing (relates 030-53/55/57). Update SPEC take operator (§13.18.9, lines 21076–21086) to cross-reference the new completion rule, and SPEC §13.18.12 to add the completed-consumer cursor paragraph. Anchor LOG: 030-134 (line 3640), 030-143 (3649), 030-188 (3694), 030-189 (3695), 030-191 (3697), 030-193 (3699), 030-196 (3702). Findings: F082 (report line 2405), F201 (report line 2502). Verify: `grep -niE 'stream is complete|is complete|completion' SPEC.md` should return a defined completion rule, not just the take comment at line 21078; `grep -n 'take\[' DECISION_LOG.md` cross-references the new rule.
...
> **For execution:** Canonical side = Model B. LOG entries to conform: 035-5 (line 4372, "fixed by member order" — keep term, now defined), 035-6 (line 4373, "declared order" — restate as the composite branch of member order), 034-8 (line 4359, "walk-order" — add/point to a LOG definition), 034-12 (line 4363, "member values in walk order"), 035-10 (line 4377, "member edges in member order"), 033-81 (line 4193, "member edges listed in member order"). New LOG decision needed: define member order (pivot) and home walk order in section 034; check numbering places it by topic (034 for walk order, 035 for member order). SPEC sections: §13.20 (walk-order def currently at SPEC 23053-23055), §13.21 (fold/member order). Compare with 017-255 (line 2396) / 017-264 (line 2405) "entry order / structural (entry) order" — state distinct, do not equate. Findings: F212 [M/undefined_term, walk order defined only in SPEC], F215 [L/vague_term, three terms no equivalence]. Depends on: K-YIELDFOLD-IR (fold-cell member-edge shape must use the same term). Verify: `grep -n 'member order\|declared order\|walk order\|walk-order' DECISION_LOG.md` should show every hit resolving to the single defined term; `grep -n 'walk order' DECISION_LOG.md` must return a defining entry, not only 034-8/034-12.
...
> **For execution:** Canonical side = umbrella-at-signature / excluded-at-read-site (confirms D-18 Model A) + Option B kind-split on the umbrella read rule. LOG edits: 016-172 (line 2025, §13.2.8) — rewrite "binding to any reactive value cell" to the wording of 016-283 (2136) / 029-124 (3503): binds any reactive cell; a `stream T` is excluded at the read site, not by the signature; coordinate with D-01's spelling pass (its list already includes 016-172, 016-283, 030-48). 016-64 (line 1917, §13.17.3.1) — add the stream carve-out: dependency edge for all kinds; auto-deref'd cell-pool read for value cells only; a bare stream name is the 030-246 error (3752, §13.18.16) in value contexts and contributes events per 030-64/65/66 (3570-3572, §13.18.7/.1) under the surrounding-declaration constraint 030-82 (3588, §13.18.7.3). Stand as-is: 016-62 (1915), 016-165 (2018), 016-283 (2136), 029-124 (3503), 030-48 (3554, spelling only per D-01), 030-246 (3752). SPEC: parameter carve-out already correct at 12415-12418; add the matching carve-out sentence to the cell-name text at §13.2.8 (12449-12459, currently value-cell examples only); umbrella + monitor example §13.18.5 (20513-20543) and read-site exclusion (20555-20557) unchanged; diagnostic at 21743-21752; observe arm-body rule at 12931-12934 unchanged, cross-reference only. Depends on: D-18 (read-site model), D-01 (spelling of the same entries). Findings: F243 (MED/contradiction/CONFIRMED, report line 125) — auto after confirmation; F245 (LOW/divergence/CONFIRMED, report line 498) — auto under Option B. Verification greps: `grep -n 'any reactive value cell' DECISION_LOG.md` (zero after edit; currently exactly 016-172); `grep -c 'excluded at the read site' DECISION_LOG.md` (2 now, 3 after — 016-172 joins 016-283/029-124); `grep -n "auto-deref'd cell-pool read" DECISION_LOG.md` (016-64 must carry the carve-out); `grep -n 'never denotes an event value' SPEC.md` (observe rule intact). Corpus pin: HEAD 1b0c6b8.
...
> **For execution:** Canonical side — Option A (add the production), `entry`-alternative placement, inside `scope`. Edit LOG-first per protocol. LOG entries to conform: 033-56 (six primitives, entry 4168), 033-68 (surface stream lowers to stream primitive, 4180), 033-106..118 (stream data-model fields, entries 4218–4230), 033-53 (text form normative, 4165), 033-168 (grammar covers graph section, 4280), 033-226 (compiler renders in text form, 4338), 033-231 (interop against text form, 4343), 028-65 (stream maps to graph-IR stream primitive, 3366). SPEC edits: §15.4.6 — add `stream` to the `entry` production (SPEC.md:24507) and a new `stream ::=` production after the `effect` production (SPEC.md:24519–24520), covering the ten fields listed at §15.4.1 (SPEC.md:24130–24154); extend the §15.4.6 prose (SPEC.md:24537–24561) with a sentence serializing a stream and add a worked stream example. If Option B is chosen instead, amend 033-53/033-226/033-231 to exclude stream and add a carve-out citing 033-55 (need-not-externalize, 4167). Finding id: P022 (addendum). Verification greps: `grep -n 'stream ::=' SPEC.md` must return one hit; `grep -n 'entry *::=' SPEC.md` must show stream in the alternatives; confirm zero `stream ::=` at HEAD 1b0c6b8 before editing.
...
> **For execution:** Canonical side: Model A (per-declaration cells). Retract/rewrite the Model-B entries: **033-170** (LOG 4282, single whole-record `pool_index<%T>` cell) and **033-202** (LOG 4314, desired-builder behavior sig/output) — either delete or demote to a reconciler-facing view assembled from per-declaration cells. Conform to the plural model already in **033-122** (LOG 4234, `desired_cell_ids` = list). Keep legal: **031-30** (LOG 3799, `f.request` read), **017-278** (LOG 2419, `f.in_flight` flat access), **031-31/032/033/036** (LOG 3800-3805, desired event-output streams), **031-37/038/039/040/041** (LOG 3806-3810, per-arm freeze). Narrow **033-210** (LOG 4322, "no activation input anywhere") to whole-effect suspend/resume only; reconcile with **033-62** (LOG 4174, `when`/`given` lower to `gate`). SPEC: rewrite §15.4.5 (SPEC ~24414-24483, worked example + "no activation input" prose) and §15.4.4 desired-builder row in the Behavior ABI table (SPEC 24394); conform §15.4.1 `desired_cell_ids` prose (SPEC 24177-24178) and the graph text grammar in §15.4.6 to emit per-declaration desired cells (incl. stream cells per K-STREAM-IR-TEXT). Surface refs: §13.19.4 (desired streams + gated arms), §13.19.5 (observed lifted-computed precedent). Findings: P023, P024, P025 (addendum). Depends on: K-STREAM-IR-TEXT (stream cell text form). Verify greps: `grep -n '033-170\.\|033-202\.\|033-122\.\|033-210\.' DECISION_LOG.md`; `grep -n 'whole-record\|desired-builder\|desired_cell_ids\|no activation input' SPEC.md`.
...
> **For execution:** Canonical side = two-axis model, exactly two sealed members (LOG 030-253 at DECISION_LOG.md:3759, 030-254 :3760, 030-257 :3763, 030-258 :3764, 030-260 :3766; SPEC §13.18.3 at SPEC.md:20428-20457, aliases 20459-20471; §13.18.8.6 at SPEC.md:20947-20953). Conform 030-37 (:3543): "one of the four `StreamPolicy` members" → "one of the two sealed `StreamPolicy` members (`Ring[N]` or `Gate[N]`)". F193 residual, if "no aliases" is confirmed: conform 030-116 (:3622) to stop naming `RecurrentRingStream[T,B,H]`/`RecurrentGateStream[T,B,H]` and restate per §13.18.8.6's actual text (produces a `Ring[B]`/`Gate[B]`-policy stream carrying history depth H); fix 030-36's (:3542) parenthetical "(likewise `GateStream`/recurrent)" → "(likewise `GateStream`)"; tighten 030-260's (:3766) tail so "the recurrent forms" cannot be read as alias names. SPEC unchanged. Related: K-CELL-MODEL (D-01) for kind-vs-bracket spellings. Findings: F199 (HIGH, main report L2280 — auto), F193 (MED, L2421 — STILL, alias existence). Verify: `grep -n 'four .StreamPolicy.\|RecurrentRingStream\|RecurrentGateStream' DECISION_LOG.md` returns zero; `grep -n 'RecurrentRing' SPEC.md` returns only the denial sentences (SPEC.md:20452, 20468); `grep -n 'RecurrentRing' DECISION_LOG.md` returns only 030-253's retirement clause.
...
> **For execution:** Canonical side = `Vec[T]` spec-named / stdlib-owned, constructor `std::vec::new()`. LOG edits: 012-129 (DECISION_LOG.md:1324) — keep "stdlib concern, no dedicated language syntax," replace "the vector's name and syntax are outside the specification" with the pinned name (`Vec[T]` in `std::vec`; API surface beyond spec-referenced members is stdlib-owned); 013-200 (:1586) — `Vec::new()` → `std::vec::new()`. Unchanged: 012-91 (:1286), 011-55 (:1166), 005-129 (:470). SPEC edits: §9.3.6 (SPEC.md:6869-6873) — drop the "(`Vec[T]`, `Vector[T]`, or whatever stdlib chooses)" hedge, state the pinned name; replace `Vec::new()` at SPEC.md:9605 (§11.10.6 — F121), 9741 (§11.11.2 — F122), 9784, 11019, 11154, 13563, 15434, 15449, 15472, 15507, 15676. Supporting facts: forbidden form 011-55 + SPEC §8.7 (SPEC.md:6267-6273) + §9.4.1.4 (SPEC.md:7166-7170); module path `std::vec::Vec` at SPEC.md:7640/7644. Cross-note: 013-200 also anchors addendum P028 (closure-field call resolution) — untouched here. Findings: F122 (MED, main report L3300), F121 (MED, L3314), F021 (LOW, L3841) — all auto. Verify: `grep -n 'Vec::new' SPEC.md DECISION_LOG.md` returns zero; `grep -n 'whatever stdlib chooses' SPEC.md` returns zero; `grep -n '^012-129\.' DECISION_LOG.md` shows the pinned name.
...
> **For execution:** Canonical side = operator identity rule as the single effect-instance rule (028-51 at DECISION_LOG.md:3352; SPEC §13.15.6 at SPEC.md:19463-19468; algorithm + tiebreak SPEC §13.17.10 at SPEC.md:20076-20092), plus keyed-repeat scope clause (018-109/018-110 :2579-2580; SPEC §13.5.4.3 at 15436-15443, §13.5.4.7 at 15596-15599), plus path-as-addressing demotion. LOG edits: 028-4 (:3305) second sentence — interpreter-placed effect instances follow the 028-51 rule; interpretation-site rooting applies to their cell paths, not their identity; 028-5 (:3306) — the ":N applies equally to interpreter-placed effects" clause becomes ordinal-as-address/tiebreak per §13.17.10's syntactic-order matching; 028-51 (:3352) — add the keyed-repeat clause (element key = per-element enclosing scope, superseding argument-binding equality; cross-ref §13.5.4.3/§13.5.4.7). Cells unchanged: 028-4 first sentence, 028-6 (:3307), SPEC §13.15.2 (19289-19314) remain the cell rule. Depends-on: K-HOOK-ORDER (D-08) for when the resulting teardown/create fire (its F108 reload-commit residual). F209 residual: repoint 028-4's effect clause §13.15.2→§13.15.6, or add effect text to §13.15.2. Findings: F205 (MED, main report L4656 — auto), F220 (MED, L4701 — auto), F209 (LOW, L4856 — STILL, repoint vs elaborate). Verify: `grep -n '^028-4\.\|^028-5\.\|^028-51\.' DECISION_LOG.md` — post-edit, no entry asserts path/ordinal as effect-instance identity; `grep -n 'element key' SPEC.md` — §13.15.6 states the key clause; confirm §13.17.10's tiebreak text (SPEC.md:20087-20092) is unchanged.
